...
describe('testing mdoc', () => {
    ...
    test('Verify reader authentication', async () => {
        ...
        const certificateDER = devRequest.mobileDocumentRequests[0].readerAuthentication.x5Chain;
        ...
        expect(await devRequest.mobileDocumentRequests[0].verify(params, coseCryptoProvider)).toBeTruthy();

        allowedToRetain.set(MDL.Namespace, new Set<string>(['family_name']));
        params = new MDocRequestVerificationParams(true, READER_KEY_ID, allowedToRetain, readerAuthentication);
        expect(await devRequest.mobileDocumentRequests[0].verify(params, coseCryptoProvider)).toBeFalsy();
    });
