...
import { CborMap } from "../src/data-element/cbor-map";
...
describe('testing mdoc', () => {
    ...
    test('Sign mDL', async () => {
        ...
        const deviceAuthentication = new DeviceAuthentication(new CborArray(new CborNil(), 
                                                                            ...
                                                                            new CborNil()), 
                                                              mdoc.docType, 
                                                              CborEncodedDataItem.encode(new CborMap(new Map<MapKey, CborDataItem>())));
    ...
    test('Verify Mdoc example by certificate in COSE header', async () => {
        ...
        for (const documentElement of documentsElement.getValue()) {
            ...
            const issuerSigned = CborDataItem.to(IssuerSigned, <CborMap>mdocMapElement.get(new MapKey('issuerSigned')));
            const deviceSigned = CborDataItem.to(DeviceSigned, <CborMap>mdocMapElement.get(new MapKey('deviceSigned')));
...
*/
    ...
    test('Array of any', () => {
        ...
        const cborArray = new CborArray(new CborNumber(1), 
                                        new CborMap(new Map<MapKey, CborDataItem>([[new MapKey('a'), new CborNumber(1)]])),
                                        new CborNil(),
