...
describe('testing mdoc', () => {
    ...
    test('Verify Mdoc example by certificate in COSE header', async () => {
        ...
        for (const documentElement of documentsElement.value) {
            ...
            const issuerSigned = Cbor.fromDataItem(<MapElement>mdocMapElement.get(new MapKey('issuerSigned')), IssuerSigned);
            const deviceSigned = Cbor.fromDataItem(<MapElement>mdocMapElement.get(new MapKey('deviceSigned')), DeviceSigned);
    ...
    test('Verify reader authentication', async () => {
        ...
        const de = CborDecoder.decode(encodedCBORElement.value);
    ...
    test('Selective disclosure', async () => {
        ...
        const listElement = <ListElement>CborDecoder.decode(new EncodedCBORElement(deviceAuthenticationBytes).decode().value);
        const deviceAuthentication = Cbor.fromDataItem(listElement, DeviceAuthentication);
...
//        )
    ...
    test('Verify device mac', async () => {
        ...
        const deviceAuthentication = Cbor.fromDataItem(<ListElement>deviceAuthenticationDataElement, DeviceAuthentication);
        expect(deviceAuthentication.dataItems[0].value.toString()).toBe("DeviceAuthentication");
